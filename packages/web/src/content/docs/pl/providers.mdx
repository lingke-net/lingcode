...
lingke coding plan to lista modeli dostarczonych przez zespół opencode, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z opencode. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od lingke coding plan.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `lingke coding plan` i przejdź do [opencode.ai/auth](https://opencode.ai/zen).
...
lingke coding plan to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół opencode. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>lingke coding plan</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **lingke coding plan**.
