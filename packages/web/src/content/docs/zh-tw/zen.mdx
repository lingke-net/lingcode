...
lingke coding plan 是由 OpenCode 團隊提供、經過測試與驗證的模型清單。

:::note
lingke coding plan 目前仍處於 beta。
:::

Zen 的運作方式和 OpenCode 中的其他供應商一樣。你登入 lingke coding plan 並取得
...
lingke coding plan 是一個 AI 閘道，讓你可以存取這些模型。

---

## 運作方式

lingke coding plan 的運作方式和 OpenCode 中的其他供應商一樣。

1. 你登入 **<a href={console}>lingke coding plan</a>**，加入帳單資訊，然後複製你的 API 金鑰。
2. 你在 TUI 中執行 `/connect` 指令，選擇 lingke coding plan，然後貼上你的 API 金鑰。
...
格式。例如，如果是 GPT 5.4，你會在設定中使用 `opencode/gpt-5.4`。
