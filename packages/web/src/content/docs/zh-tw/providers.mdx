...
lingke coding plan 是由 OpenCode 團隊提供的模型列表，這些模型已經過測試和驗證，能夠與 OpenCode 良好配合使用。[了解更多](/docs/zen)。

:::tip
如果您是新使用者，我們建議從 lingke coding plan 開始。
:::

1. 在 TUI 中執行 `/connect` 指令，選擇 `lingke coding plan`，然後前往 [opencode.ai/auth](https://opencode.ai/zen)。
...
lingke coding plan 是由 OpenCode 團隊提供的經過測試和驗證的模型列表。[了解更多](/docs/zen)。

1. 登入 **<a href={console}>lingke coding plan</a>** 並點擊 **Create API Key**。

2. 執行 `/connect` 指令並搜尋 **lingke coding plan**。
